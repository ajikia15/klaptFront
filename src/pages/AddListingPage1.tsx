  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        // Construct laptop object for backend
        const laptopData = {
          title: value.title,
          ...
          brand: value.brand,
          model: value.model,
          graphicsType: value.graphicsType as "Integrated" | "Dedicated",
          gpuBrand:
            value.graphicsType === "Dedicated"
              ? value.gpuBrand || undefined
              : undefined,
          gpuModel:
            value.graphicsType === "Dedicated"
          ...
          vram:
            value.graphicsType === "Dedicated" && value.vram
              ? parseInt(value.vram)
              : undefined,
          backlightType: value.backlightType,
          processorBrand: value.processorBrand as "Intel" | "AMD" | "Apple",
          processorModel: value.processorModel,
          cores: parseInt(value.cores),
          threads: parseInt(value.threads),
          ram: parseInt(value.ram),
          ramType: value.ramType as "DDR3" | "DDR4" | "DDR5",
          storageType: value.storageType as "HDD" | "SSD" | "HDD + SSD",
          ...
          tag: value.tags && value.tags.length > 0 ? value.tags : undefined,
          condition: value.condition as "new" | "like-new" | "used" | "damaged",
          stockStatus: value.stockStatus as "reserved" | "sold" | "in stock",
        };
